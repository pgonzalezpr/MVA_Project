...
After pruning, the trees using both sets of variables are the identical for Brisbane.
...
The model improves by lowering the probability threshold to 20%
...
pred_test2_prob20 <- ifelse(pred_test2_prob[,2]>0.2,"Yes","No") 
confusionMatrix(data= as.factor(pred_test2_prob20), test2$RainTomorrow, positive="Yes")
```
...
# predict rain if chance of rain is more than 75% (default is 50%)
pred_trainBal2_prob75 <- ifelse(pred_trainBal2_prob[,2]>0.5,"Yes","No") 
confusionMatrix(data= as.factor(pred_trainBal2_prob75), train2$RainTomorrow, positive="Yes")
```
```{r}
#Test Set Evaluation
#Confusion matrix-test
...
We are able to slightly improve the accuracy and specificity of the model by using a different probability rule, however sensitivity decreases. We predict there will be rain if the chance of rain is more than 75%. The default used by the "class" predictor is 50%.
...
# predict rain if chance of rain is more than 75% (default is 50%)
pred_testBal2_prob75 <- ifelse(pred_testBal2_prob[,2]>0.75,"Yes","No") 
