...
df <- read.csv("Location CSVs/df_Cairns.csv",sep=",",stringsAsFactors = T)
df <- subset(df, Location == "Cairns")
...
Density plots of imputed variables match density plots of original variables with NA's removed.

```{r}
#Create copy of original data before merging the imputations.
df_original <- df
...
cor_matrix = cor(df[,c(7:11,13,16:25,28)]) #starting from column 4: MinTemp
...
df6$Season <- ifelse(df6$Month %in% c("mayo","junio","julio",
                                      "agosto","septiembre","octubre"), "dry","wet")
...
sunshine <- sunshine + geom_point(data = subset(df6, Date %in% missing_Dates$Date), 
...
evaporation <- evaporation + geom_point(data = subset(df6, Date %in% missing_Dates$Date), 
...
pressure9 <- pressure9 + geom_point(data = subset(df6, Date %in% missing_Dates$Date), 
...
pressure3 <- pressure3 + geom_point(data = subset(df6, Date %in% missing_Dates$Date), 
...
humidity9 <- humidity9 + geom_point(data = subset(df6, Date %in% missing_Dates$Date), 
...
temp9 <- temp9 + geom_point(data = subset(df6, Date %in% missing_Dates$Date), 
...
wind9 <- wind9 + geom_point(data = subset(df6, Date %in% missing_Dates$Date), 
...
It seems that performing MICE for the missing months parting from the available variables + TempRange has resulted in an acceptable imputation, as the values tend to follow the time series' trends. Thus, we'll output the completed dataset to a .csv file and use it to perform our analysis.

```{r}
#write.csv(df6,'Completed_Location_CSVs/df_Cairns_completed.csv', row.names = FALSE)
```
